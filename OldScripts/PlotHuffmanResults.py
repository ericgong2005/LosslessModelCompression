...
def plot_grouped_bars(
    ...
    if sub.empty:
        print(f"Skipping empty plot: split={split_name}, metric={metric_col}, group={group_col}")
        return
    ...
    if pivot.empty:
        print(f"Skipping fully empty pivot: split={split_name}, metric={metric_col}, group={group_col}")
        return
    ...
    ax.set_title(f"{metric_label} by {group_label} | Split={split_name} | Huffman")
...
def main():
    ...
    required_columns = ["format", "variance", "split", "algorithm"]
    ...
    df = coerce_numeric(df, list(METRICS.keys()))
    ...
    for split_name in SPLITS_TO_PLOT:
        for metric_col, metric_label in METRICS.items():
            ...
            plot_grouped_bars(
                ...
                algo_col="algorithm",
                output_path=OUTPUT_DIR / f"{metric_col}_by_format_split_{split_name}.png",
            )
            ...
            plot_grouped_bars(
                ...
                algo_col="algorithm",
                output_path=OUTPUT_DIR / f"{metric_col}_by_variance_split_{split_name}.png",
            )
