...
MAX_WORKERS = 6                    # safe for 69 GB node; raise if RAM allows

_TABLE_BYTES = 0   # set to 256 to include canonical header overhead
...
_SINGLE_ALGOS = ["raw_huffman", "gorilla_base", "xor_delta", "gorilla_xor_delta"]
_MULTI_ALGOS  = ["byte_transpose", "semantic_sep", "bitplane"]
...
def _compressed_bytes_from_bits(bits: int) -> int:
    return _TABLE_BYTES + math.ceil(bits / 8)
